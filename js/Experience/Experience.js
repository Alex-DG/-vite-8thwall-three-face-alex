...
import softEyeMouthSrc from '../../assets/alpha/soft-eyes-mouth.png'

import headOccluderSrc from '../../assets/models/head-occluder.glb'
import stereoGlassesSrc from '../../assets/models/stereo-glasses.glb'
import alexSrc from '../../assets/models/alex.glb'

import tatSrc from '../../assets/tattoos/8-tat.png'
import uvBlackSrc from '../../assets/tattoos/uv-black.png'
import uvBrightSrc from '../../assets/tattoos/uv-bright.png'
...
const buildGlasses = (loader, head) => {
  const glasses = new THREE.Object3D()

  // add frame glb.
  loader.load(stereoGlassesSrc, (glassesObj) => {
    ...
    noseAttachment_ = noseAttachment
  })
}
...
const buildHead = (modelGeometry) => {
  ...
  // console.log({ basicMaterial: XRExtras.ThreeExtras.basicMaterial })
  ...
  const headMesh = XRExtras.ThreeExtras.faceMesh(
    ...
    XRExtras.ThreeExtras.basicMaterial({
      opacity: 0.8,
      tex: uvBlackSrc,
      // tex: new URL('../../assets/tattoos/8-tat.png', import.meta.url),
      alpha: softEyeMouthSrc,
      // alpha: new URL('../../assets/alpha/soft-eyes-mouth.png', import.meta.url),
    })
  )
  ...
  // buildGlasses(loader, head)

  loader.load(alexSrc, (glb) => {
    alex_ = glb.scene
    alex_.scale.multiplyScalar(8)
    alex_.rotateY((Math.PI / 2) * 2)
    alex_.position.y = -1
    head.add(alex_)
  })
  ...
  // Add occluder.
  // const occluder = await loader.loadAsync(headOccluderSrc)
  ...
  // loader.load(headOccluderSrc, (occluder) => {
  //   occluder.scene.scale.set(1.1, 1.1, 1.4)
  //   occluder.scene.position.set(-0.02, 0, 0.25)
  //   occluder.scene.traverse((node) => {
  //     if (node.isMesh) {
  ...
  const show = (event) => {
    ...
    noseAttachment_?.position.copy(attachmentPoints.noseBridge.position)
